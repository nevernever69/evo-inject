#!/usr/bin/env python3
"""
main_noseed.py - Zero-Seed Ablation Experiment

Same architecture as main.py but with NO attack seed phrases.
The phrase library starts with only neutral filler phrases.
All attack strategies must be discovered purely through:
  - Token block evolution (GP)
  - Loss-guided hill climbing (refinement)
  - MAP-Elites diversity pressure

This is the key experiment: can a purely evolutionary process,
without any human knowledge of injection patterns, discover
adversarial token sequences that break LLM restrictions?

If YES → strong NeurIPS contribution (novel discovery mechanism)
If NO  → still publishable as negative result + comparison baseline
"""
...
from config import (
    ...
    TOKEN_BLOCK_SIZE, TOKEN_BLOCK_MIN, TOKEN_BLOCK_MAX,
)
...
from gp import Program, program_complexity, extract_token_ids
...
# Only neutral connecting words. Zero injection patterns.
NOSEED_PHRASES = {
    'filler': [
        "Please",
        "Now",
...
class EmptyPhraseLibrary(PhraseLibrary):
    """
    Phrase library with NO attack seed phrases.
    Only neutral fillers. All attack phrases must be discovered
    through token block evolution and promotion.
    """

    def init_from_seeds(self, tokenizer):
        """Initialize with only neutral filler phrases."""
        self._tokenizer = tokenizer

        for cat, phrases in NOSEED_PHRASES.items():
            for text in phrases:
                tokens = tokenizer.encode(text, add_special_tokens=False)
                if len(tokens) > config.MAX_PHRASE_TOKENS:
                    tokens = tokens[:config.MAX_PHRASE_TOKENS]
                self._add_phrase(text, tokens, cat, is_seed=True)
...
def main():
    parser = argparse.ArgumentParser(
        description='Zero-Seed Ablation: Compositional Evolutionary Attack Search'
    )
    ...
    print("=" * 60)
    print("  ZERO-SEED ABLATION EXPERIMENT")
    print("  NO attack seed phrases — discovery from scratch")
    ...
    print()
    print("Starting ZERO-SEED evolutionary attack search...")
    print("-" * 60)
    ...
    # ── Final Report ──
    print("\n" + "=" * 60)
    print("  FINAL REPORT (ZERO-SEED ABLATION)")
    print("=" * 60)
    ...
    report_path = os.path.join(
        logger.log_dir, f'final_report_noseed_{int(time.time())}.json'
    )
