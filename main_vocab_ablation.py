#!/usr/bin/env python3
"""
main_vocab_ablation.py - Vocabulary Size & Composition Ablation

Parameterized experiment for Option B: "Understanding the Search Space
of Gradient-Free Adversarial Optimization."

Same architecture as main_constrained.py (zero attack seeds, constrained
token generation), but with configurable:
  --vocab-size N     Target pool size (default 5000)
  --pool TYPE        Pool construction method:
                       curated  - proportional subsample of interesting/separator/special
                       random   - random sample from full 128K vocab
                       interesting - INTERESTING_TOKENS only
                       separator - SEPARATOR_TOKENS only
                       special  - SPECIAL_TOKENS only
                       int_sep  - INTERESTING + SEPARATOR (no special)
  --seed N           Random seed for reproducible pool construction

This enables the vocab size ablation grid:
  500, 1K, 2K, 5K, 10K, 20K, 50K curated pools
  5K random pools (multiple seeds)
  Composition-only pools (interesting, separator, special, int_sep)
"""
...
def build_pool(pool_type, vocab_size, seed, interesting, separator, special, all_tokens):
    """
    Build a custom token pool based on pool_type and vocab_size.

    Returns:
        pool: list[int] — token IDs to use
        pool_info: dict — metadata about pool composition
    """
    rng = random.Random(seed)

    if pool_type == 'interesting':
        pool = list(interesting)
        return pool, {
            'type': 'interesting',
            'size': len(pool),
            'interesting': len(pool),
            'separator': 0,
            'special': 0,
            'random_fill': 0,
        }

    elif pool_type == 'separator':
        pool = list(separator)
        return pool, {
            'type': 'separator',
            'size': len(pool),
    ...
    elif pool_type == 'int_sep':
        pool = list(set(interesting + separator))
        return pool, {
            'type': 'int_sep',
            'size': len(pool),
        ...
        # Random sample from full vocabulary
        if vocab_size >= len(all_tokens):
            pool = list(all_tokens)
        else:
            pool = rng.sample(all_tokens, vocab_size)
        return pool, {
            'type': 'random',
            'size': len(pool),
            'interesting': len(set(pool) & set(interesting)),
            'separator': len(set(pool) & set(separator)),
            'special': len(set(pool) & set(special)),
            'random_fill': len(pool),
            'seed': seed,
        }

    elif pool_type == 'curated':
        # Proportional subsample from curated pools, fill remainder from vocab
        curated_total = len(interesting) + len(separator) + len(special)

        if vocab_size >= curated_total:
            # Use all curated tokens + random fill from remaining vocab
            pool = list(set(interesting + separator + special))
            remaining_needed = vocab_size - len(pool)
            if remaining_needed > 0:
                # Fill from tokens NOT in the curated set
                curated_set = set(pool)
                candidates = [t for t in all_tokens if t not in curated_set]
                if candidates:
                    fill = rng.sample(candidates, min(remaining_needed, len(candidates)))
                    n_random = len(fill)
                    pool.extend(fill)
                else:
                    n_random = 0
            ...
            return pool, {
                ...
                'separator': len(separator),
                'special': len(special),
                'random_fill': n_random,
            }
        else:
            # Subsample proportionally from each curated category
            ratio_int = len(interesting) / max(curated_total, 1)
            ratio_sep = len(separator) / max(curated_total, 1)
            ratio_spc = len(special) / max(curated_total, 1)

            n_int = max(1, int(vocab_size * ratio_int))
            n_sep = max(1, int(vocab_size * ratio_sep))
            n_spc = max(1, int(vocab_size * ratio_spc))

            # Adjust to hit target size
            total = n_int + n_sep + n_spc
            if total < vocab_size:
                n_int += vocab_size - total
            elif total > vocab_size:
                excess = total - vocab_size
                n_int = max(1, n_int - excess)

            sampled_int = rng.sample(interesting, min(n_int, len(interesting)))
            sampled_sep = rng.sample(separator, min(n_sep, len(separator)))
            sampled_spc = rng.sample(special, min(n_spc, len(special)))

            pool = list(set(sampled_int + sampled_sep + sampled_spc))
            return pool, {
                'type': 'curated',
                'size': len(pool),
                'interesting': len(sampled_int),
                'separator': len(sampled_sep),
                'special': len(sampled_spc),
                'random_fill': 0,
            }
...
def main():
    parser = argparse.ArgumentParser(
        description='Vocab Ablation: Parameterized Search Space Experiment'
    )
    ...
    # Vocab ablation parameters
    parser.add_argument('--vocab-size', type=int, default=5000,
                        help='Target token pool size (default 5000)')
    parser.add_argument('--pool', type=str, default='curated',
                        choices=['curated', 'random', 'interesting',
                                 'separator', 'special', 'int_sep'],
                        help='Pool construction method')
    parser.add_argument('--seed', type=int, default=42,
                        help='Random seed for pool construction and reproducibility')
    ...
    pool_label = f"{args.pool}-{args.vocab_size}" if args.pool in ('curated', 'random') else args.pool

    print("=" * 60)
    print("  VOCABULARY ABLATION EXPERIMENT")
    ...
    # Enable constrained mode globally
    import gp as _gp_module
    _gp_module.CONSTRAINED_MODE = True

    # Build custom pool
    from gp import INTERESTING_TOKENS, SEPARATOR_TOKENS, SPECIAL_TOKENS, TOKEN_POOL
    pool, pool_info = build_pool(
        pool_type=args.pool,
        vocab_size=args.vocab_size,
        seed=args.seed,
        interesting=INTERESTING_TOKENS,
        separator=SEPARATOR_TOKENS,
        special=SPECIAL_TOKENS,
        all_tokens=TOKEN_POOL,
    )

    # Set the custom pool globally
    _gp_module.CUSTOM_POOL = pool

    print(f"  Pool constructed: {len(pool)} tokens")
    print(f"    interesting: {pool_info['interesting']}")
    print(f"    separator:   {pool_info['separator']}")
    print(f"    special:     {pool_info['special']}")
    print(f"    random_fill: {pool_info.get('random_fill', 0)}")
    ...
    metrics = Metrics()
    logger = Logger()
    detailed = DetailedLogger(
        log_dir=logger.log_dir,
        prefix=f'detailed_vocab_{args.pool}_{len(pool)}',
    )
    ...
    wandb_config = {
        ...
        'vocab_size_actual': len(pool),
        'pool_interesting': pool_info['interesting'],
        'pool_separator': pool_info['separator'],
        'pool_special': pool_info['special'],
        'pool_random_fill': pool_info.get('random_fill', 0),
        'pool_seed': args.seed,
        'archive_cells': archive.total_cells(),
        'compute_loss': compute_loss,
        'do_refine': do_refine,
        'device': config.MODEL_DEVICE,
        'version': 'v3-vocab-ablation',
    }
    wb = WandbLogger(
    ...
    print()
    print(f"Starting VOCAB ABLATION search ({pool_label})...")
    print("-" * 60)
    ...
    # ── Final Report ──
    print("\n" + "=" * 60)
    print(f"  FINAL REPORT (VOCAB ABLATION: {pool_label})")
    print("=" * 60)
    ...
    print(f"  Pool: {pool_label} ({len(pool)} tokens)")
    ...
    report_path = os.path.join(
        logger.log_dir,
        f'final_report_vocab_{args.pool}_{len(pool)}_{int(time.time())}.json',
    )
